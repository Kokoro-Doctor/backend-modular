...
This module manages the persistent link between a patient and a doctor
that survives subscription expiry.
"""
...
def create_relation(
    ...
    - If ACTIVE exists: refresh metadata (subscription_id, type, linked_by).
    - If only INACTIVE exists: reactivate the most recently updated row.
    - Else: insert a new relation.
    """
    ...
    if active:
        expr_parts = ["#u = :u"]
        expr_names: dict[str, str] = {"#u": "updated_at"}
