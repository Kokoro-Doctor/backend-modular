"""
Groq API error handling - converts API exceptions to HTTPException.
...
def to_http_exception(e: Exception, log_prefix: str = "") -> HTTPException:
    """
    Convert Groq/API exception to HTTPException.
    ...
    if isinstance(e, APIError):
        ...
        logger.error(f"{log_prefix} Groq API error: {type(e).__name__}: {e}")
        return HTTPException(
            status_code=503,
